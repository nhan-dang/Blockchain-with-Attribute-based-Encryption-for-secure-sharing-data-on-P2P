...
class Blockchain:
    ...
    def verify_transaction_signature(self, sender_address,
        ...
        verifier = sig_PKCS.new(public_key)
        h = SHA.new(str(transaction).encode('utf8'))
        return verifier.verify(h, binascii.unhexlify(signature))

    def submit_transaction(self, sender_address,
                           recipient_address, iv, sym_key, value, signature):
        """
    ...
    def valid_proof(self, transactions,
                    last_hash, nonce, difficulty=MINING_DIFFICULTY):
        """
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            transactions = [OrderedDict(
                (k, transaction[k]) for k in
                transaction_elements) for
...
@app.route('/ipfs/push')
def ipfs_push():
    return render_template('./ipfs_push.html')
...
def new_transaction():
    ...
    required = ['sender_address', 'recipient_address', 'iv',
                'sym_key', 'abe_private_key', 'signature']
    if not all(k in values for k in required):
        return 'Missing values', 400
    # Create a new Transaction
    transaction_result = blockchain.submit_transaction(
        values['sender_address'], values['recipient_address'], values['iv'],
        values['sym_key'], values['abe_private_key'], values['signature'])
...
"""
Add route to encrypt and decrypt
"""


@app.route('/private_key/revoke', methods=['POST'])
def private_key_revoke():
    input_text = request.form
    policy_str = input_text['public_key']

    mode = 'a' if os.path.exists(file_path) else 'w'
    with open(file_path, mode) as f:
        f.write(policy_str + '\n')
    response = {'msg': 'New negative attribute has been added'}
    return jsonify(response), 200


@app.route('/private_key/request_revoke', methods=['GET'])
def private_key_request_revoke():
    with open(file_path, 'r') as f:
        negative_attributes = f.readlines()
    response = {'negative_attributes_list': negative_attributes}
    return jsonify(response), 200


"""
push encrypted data to ipfs
"""
...
@app.route('/ipfs/push/encrypted', methods=['POST'])
def ipfs_push_encrypted():
    input_files = request.files
    encrypted_data_file = input_files['encrypted_file']
    fe_filename = secure_filename(encrypted_data_file.filename)
    encrypted_data_file.save(os.path.join(app.config['UPLOAD_FOLDER'],
                                          fe_filename))

    ipfs_api = connect_to_ipfs()
    result = push_content(ipfs_api,
                          os.path.join(app.config['UPLOAD_FOLDER'],
                                       fe_filename))

    response = {'msg': result['Hash']}
    return jsonify(response), 200
